<template>
  <template v-if="selectArr[0]">
    <FilterSelect
      ...
      :parent-title="title"
      @user-selectData="totalData"
    ></FilterSelect>
  </template>
  <template v-else>
    <div class="content-box w-auto">尚未有資料</div>
  </template>
...
const doAxios = async (group, startdate, stopdate, name, status, cur, page) => {
  // get axios data
  let href =
    "http://ec2-34-221-251-1.us-west-2.compute.amazonaws.com:8080/punch";

  try {
    let { data } = await axios.get(href, {
      params: { group, startdate, stopdate, name, status, cur, page },
    });
